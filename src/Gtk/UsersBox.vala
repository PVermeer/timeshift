...
class UsersBox : Gtk.Box{
	...
	private void init_btrfs_home_option(Gtk.Box box){
		...
		if (restore_mode){
			...
			chk_include_btrfs_home = new Gtk.CheckButton.with_label(_(@"Restore home ($(App.btrfs_home)) subvolume"));
			...
			chk_include_btrfs_home = new Gtk.CheckButton.with_label(_(@"Include home ($(App.btrfs_home)) subvolume in backups"));
